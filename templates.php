<?php
    ...
    // get table cols
    $tableCols = readTableColumns($_SESSION['missionInfo']->mykey, 'templates');

    // get table cols info
    $tableColInfo = readTableColumnInfo($_SESSION['missionInfo']->mykey, 'templates');
    ...
    $MAIN_QUERY = 'SELECT * FROM `templates` WHERE 1 ORDER BY `templates`.`id` DESC LIMIT 25';
    ...
    */
    ...
    if (isset($_GET['val'])) {
        ...
        $MAIN_QUERY = 'SELECT * FROM `templates` WHERE '.$MAIN_QUERY.' ORDER BY `templates`.`'.$_GET['sortCol'].'` '.$_GET['sortDir'].$limitStr;
    }
...
function getEditableClassAndType(colName) {
    switch (colName) {
        ...
        case 'text':
            return ['textAREAEditable googleMessage', 'textarea'];
            break;
...
function make_table(data) {
    ...
    for (let i=0; i<data.length; i++) {
        ...
        for (let j = 0; j < row.length; j++) {
            ...
            if (col == 'text') {
                html_data += '<td><div data-name="'+col+'" class="'+cls+'" data-type="'+typ+'" data-pk="'+row[1]+'">'+cell+'</div></td>';
            } else {
...
$('#employee_data').editable({ // textEditable class is now editable
    ...
    url: "templates_table/update.php",
    title: 'Edit Cell',
    type: "GET",
    dataType: 'json'
});
$('#employee_data').editable({ // textAREAEditable class is now editable
    container: 'body',
